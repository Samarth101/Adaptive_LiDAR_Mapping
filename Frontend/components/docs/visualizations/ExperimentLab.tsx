  ...
  return (
    <div className="flex-1 bg-background rounded-xl border border-border flex overflow-hidden shadow-2xl relative">
      ...
      <div className="w-1/3 bg-card border-r border-border p-6 flex flex-col relative z-10">
        <h3 className="text-xl font-normal text-foreground flex items-center gap-2 mb-6">
          <Settings2 className="w-5 h-5 text-primary" /> Pipeline Parameters
        </h3>

        <div className="space-y-6">
          <div>
            <label className="text-xs font-normal text-muted-foreground/70 tracking-normal mb-2 block">Spatial Resolution</label>
            <div className="flex bg-background rounded-lg p-1 border border-border">
              ...
              <button 
                ...
                className={`flex-1 py-2 text-sm rounded transition-all ${useAdaptive ? 'bg-primary/20 text-primary border border-primary/30 shadow-[0_0_10px_rgba(6,182,212,0.1)]' : 'text-muted-foreground/70 hover:text-muted-foreground'}`}
              >
                Foveated Adaptive
              </button>
            </div>
          ...
          <div>
            <label className="text-xs font-normal text-muted-foreground/70 tracking-normal mb-2 block">Semantic Model</label>
          ...
          <div>
            <label className="text-xs font-normal text-muted-foreground/70 tracking-normal mb-2 block">Object Detection</label>
            <button 
              onClick={() => setUseDBSCAN(!useDBSCAN)}
              className={`w-full py-2 text-sm rounded transition-all border ${useDBSCAN ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-background text-muted-foreground/70 border-border hover:text-muted-foreground'}`}
            >
      ...
      <div className="w-2/3 bg-background p-8 flex flex-col justify-center">
        ...
        <h4 className="text-2xl font-normal text-foreground mb-8 text-center">Simulated Performance Impact</h4>
