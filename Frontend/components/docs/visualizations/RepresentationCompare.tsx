  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(buffer => {
        ...
        const numPoints = Math.min(data.length / 4, 20000); // downsample for smooth animation
...
function Grid25D({ mode, isPaused }: { mode: RepMode, isPaused: boolean }) {
    ...
    <group ref={groupRef}>
      {gridCells.map((cell, i) => (
        <mesh key={i} position={[cell.position[0], cell.position[1], cell.position[2]]}>
          ...
          <meshStandardMaterial color="#ebebeb" transparent opacity={0.6} wireframe />
        </mesh>
      ))}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.1, 0]}>
        <planeGeometry args={[32, 32, 16, 16]} />
        <meshStandardMaterial color="#343434" wireframe />
      </mesh>
    </group>
  );
}
...
export default function RepresentationCompare() {
  ...
  return (
    <div className="flex-1 bg-card rounded-xl border border-border overflow-hidden relative shadow-2xl flex flex-col">
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex bg-background p-1 rounded-full border border-border">
        <button 
          onClick={() => setMode('3D')}
          className={`px-6 py-2 rounded-full font-normal transition-all text-sm ${
            mode === '3D' ? 'bg-primary text-slate-950 shadow-[0_0_15px_rgba(6,182,212,0.5)]' : 'text-muted-foreground hover:text-white'
          }`}
        >
          3D Point Cloud
        </button>
        <button 
          onClick={() => setMode('2D')}
          className={`px-6 py-2 rounded-full font-normal transition-all text-sm ${
            mode === '2D' ? 'bg-primary text-slate-950 shadow-[0_0_15px_rgba(6,182,212,0.5)]' : 'text-muted-foreground hover:text-white'
          }`}
        >
          2D Flattened Grid
        </button>
        <button 
          ...
          className={`px-6 py-2 rounded-full font-normal transition-all text-sm ${
            mode === '2.5D' ? 'bg-primary text-slate-950 shadow-[0_0_15px_rgba(6,182,212,0.5)]' : 'text-muted-foreground hover:text-white'
          }`}
        >
          2.5D Adaptive Map
        </button>
      ...
      <div className="flex-1 relative">
        <Canvas dpr={[1, 2]} camera={{ position: [0, 15, 20], fov: 50 }}>
          ...
          <gridHelper args={[40, 40, '#4d4d4d', '#2b2b2b']} position={[0, -2, 0]} />
          ...
          <Grid25D mode={mode} isPaused={isPaused} />
        </Canvas>
      </div>

      <div className="absolute bottom-4 left-4 right-4 z-10 bg-card/80 backdrop-blur-md border border-border p-4 rounded-lg">
